...
Schnabel puts hyperparameter search inside the uncertainty loop, prescribes a "search budget of at least 40 iterations"
(50 in the paper's own runs), and asks for well-tuned baselines; Ferrari Dacrema et al. supply the matched-baseline half: "11 out of the 12 reproducible neural approaches can
...
far inside that prescribed budget, so selection pressure is modest and the residual belongs in a limitation string; and because
