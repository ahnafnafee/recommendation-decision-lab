...
(arXiv:2403.03952), and this repository's `QUERY_SOURCES` consumes exactly those processed pairs from blair-bench ("shopping queries people typed, judged against catalogue items").
...
the repository's query sidecar reuses the middle link (ESCI "Exact" pairs plus first-person Amazon-C4), which is why product titles and listing text, not raw review prose, carry the query arm's
