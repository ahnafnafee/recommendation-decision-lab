...
The diagnostics show two distinct limits. The eventual target was in the promotable shortlist for 33.21% of Musical Instruments requests, while unconstrained lexical search of earlier review prose retrieved it for 2.42% of requests with usable wording. Those marginal rates have different denominators and cannot be multiplied into a measured joint ceiling. An exact target title reached its product in 99.98% of eligible cases, which checks the index on target-informed wording; mixed external query pairs reached it in 47.72% of 681 matched cases. That external set combines observed ESCI searches with semi-synthetic Amazon-C4 rewrites of target-item reviews. Its positive Musical Instruments lexical delta of +0.002394 came entirely from C4 (+0.002538); the ESCI contribution was −0.000144. These are source contributions to the full-split average, and the selected target-linked pairs do not establish general benefit from real user wording.
