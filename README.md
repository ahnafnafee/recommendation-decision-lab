...
On the full test splits the scored arm did not clear the gate. The selected confined configuration (phrase weight 0.25, repulsion 0, price penalty 0.5) lost `−0.001678` NDCG@10 against the same frozen base on Musical Instruments (35,905 requests, 95% interval `−0.002318` to `−0.000980`) and `−0.002858` on the exploratory Video Games split (35,562 requests, `−0.003598` to `−0.002195`), so it activates in neither category; the encoded variant that re-scores the same shortlist is directionally positive on the validation cohort but its interval straddles zero, and on the full Musical Instruments test split it loses `−0.001282` NDCG@10 (`−0.001944` to `−0.000577`), so the gate stays closed on both routes. The decomposition says why, and the two ceilings multiply: on the Musical Instruments test split the frozen behavioural route had never considered the eventual answer in about two-thirds of requests (shortlist coverage 33.21%), so a confined phrase had little to promote; and read back unconstrained, a person's own earlier words named the eventual answer's product in 2.42% of the requests where that person had wording to read, where the exact product title reached it in 99.98%. The test-split aggregates are committed as [musical_phrase_exploratory.json](reports/musical_phrase_exploratory.json), [video_games_phrase_exploratory.json](reports/video_games_phrase_exploratory.json) and [musical_phrase_dense_exploratory.json](reports/musical_phrase_dense_exploratory.json); the [phrase-to-products note](reports/phrase_to_products.md) holds the protocol, the six wording conditions and the reach table, and the [literature trail](reports/phrase_to_products_survey.md) holds the sources behind the design.
