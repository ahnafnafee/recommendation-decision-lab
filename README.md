...
On the full test splits the selected confined arm did not clear the gate. It lost `−0.001678` NDCG@10 against the frozen base on Musical Instruments (35,905 requests, 95% interval `−0.002318` to `−0.000980`) and `−0.002858` on Video Games (35,562 requests, `−0.003598` to `−0.002195`). The encoded variant was directionally positive on Musical Instruments validation but its interval crossed zero; on the full test split it lost `−0.001282` (`−0.001944` to `−0.000577`). These are exploratory comparisons because both category test periods had already been opened by earlier work.

The diagnostics show two distinct limits. The eventual target was in the promotable shortlist for 33.21% of Musical Instruments requests, while unconstrained lexical search of earlier review prose retrieved it for 2.42% of requests with usable wording. Those marginal rates have different denominators and cannot be multiplied into a measured joint ceiling. An exact target title reached its product in 99.98% of eligible cases, which checks the index on target-informed wording; mixed external query pairs reached it in 61.57% of 1,983 matched cases. That external set combines the full official ESCI Shopping Queries Dataset (US locale, exact-match or acceptable-substitute judgements) with semi-synthetic Amazon-C4 rewrites of target-item reviews. Its positive Musical Instruments lexical delta of +0.004931 splits +0.002500 from the ESCI pairs (1,512 requests) and +0.002431 from the C4 rewrites (471 requests): at full scale both sources contribute positively. These are source contributions to the full-split average, and the selected target-linked pairs do not establish general benefit from real user wording.

The test-split aggregates are committed as [musical_phrase_exploratory.json](reports/musical_phrase_exploratory.json), [video_games_phrase_exploratory.json](reports/video_games_phrase_exploratory.json) and [musical_phrase_dense_exploratory.json](reports/musical_phrase_dense_exploratory.json); the [phrase-to-products note](reports/phrase_to_products.md) holds the protocol, source-separated external-pair result, six wording conditions and reach table, and the [literature trail](reports/phrase_to_products_survey.md) holds the sources behind the design.
...
- An arm that was measured and declined: on the full test split of both categories a wording-driven challenger lost against the same frozen behavioural base, so the sentence answers a request on its own terms instead of displacing a route that has not earned the change.
